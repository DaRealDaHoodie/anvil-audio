...
class ConditionedSequential(nn.Module):
    ...
    def forward(self, x: Tensor, mapping: tp.Optional[Tensor] = None):
...
def default(val: tp.Optional[T], d: tp.Union[tp.Callable[..., T], T]) -> T:
    if exists(val):
        return val
    return d() if isfunction(d) else d


def exists(val: tp.Optional[T]) -> T:
...
def group_dict_by_prefix(prefix: str, d: tp.Dict) -> tp.Tuple[tp.Dict, tp.Dict]:
    return_dicts: tp.Tuple[tp.Dict, tp.Dict] = ({}, {})
    for key in d.keys():
...
def groupby(prefix: str, d: tp.Dict, keep_prefix: bool = False) -> tp.Tuple[tp.Dict, tp.Dict]:
...
class ConvBlock1d(nn.Module):
    ...
    def forward(
        self, x: Tensor, scale_shift: tp.Optional[tp.Tuple[Tensor, Tensor]] = None, causal=False
    ) -> Tensor:
        x = self.groupnorm(x)
...
class MappingToScaleShift(nn.Module):
    ...
    def forward(self, mapping: Tensor) -> tp.Tuple[Tensor, Tensor]:
...
class ResnetBlock1d(nn.Module):
    def __init__(
        ...
        *,
        ...
        use_snake: bool = False,
        num_groups: int = 8,
        context_mapping_features: tp.Optional[int] = None,
    ) -> None:
        super().__init__()
    ...
    def forward(self, x: Tensor, mapping: tp.Optional[Tensor] = None, causal=False) -> Tensor:
...
class AttentionBase(nn.Module):
    ...
    def forward(
        self, q: Tensor, k: Tensor, v: Tensor, mask: tp.Optional[Tensor] = None, is_causal: bool = False
    ) -> Tensor:
        # Split heads
...
class Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        num_heads: int,
        out_features: tp.Optional[int] = None,
        context_features: tp.Optional[int] = None,
        causal: bool = False,
    ):
        super().__init__()
    ...
    def forward(
        self,
        x: Tensor,  # [b, n, c]
        context: tp.Optional[Tensor] = None,  # [b, m, d]
        context_mask: tp.Optional[Tensor] = None,  # [b, m], false is masked,
        causal: tp.Optional[bool] = False,
    ) -> Tensor:
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        self,
        x: Tensor, *,
        context: tp.Optional[Tensor] = None,
        context_mask: tp.Optional[Tensor] = None,
        causal: tp.Optional[bool] = False
    ) -> Tensor:
...
class Transformer1d(nn.Module):
    ...
    def forward(self, x: Tensor, *, context: tp.Optional[Tensor] = None, context_mask: tp.Optional[Tensor] = None, causal=False) -> Tensor:
        x = self.to_in(x)
...
class DownsampleBlock1d(nn.Module):
    def __init__(
        ...
        *,
        ...
        attention_heads: tp.Optional[int] = None,
        attention_features: tp.Optional[int] = None,
        attention_multiplier: tp.Optional[int] = None,
        context_mapping_features: tp.Optional[int] = None,
        context_embedding_features: tp.Optional[int] = None,
    ):
        super().__init__()
    ...
    def forward(
        ...
        x: Tensor,
        *,
        mapping: tp.Optional[Tensor] = None,
        channels: tp.Optional[Tensor] = None,
        embedding: tp.Optional[Tensor] = None,
        embedding_mask: tp.Optional[Tensor] = None,
        causal: tp.Optional[bool] = False
    ) -> tp.Union[tp.Tuple[Tensor, tp.List[Tensor]], Tensor]:
...
class UpsampleBlock1d(nn.Module):
    ...
    def forward(
        ...
        x: Tensor,
        *,
        skips: tp.Optional[tp.List[Tensor]] = None,
        mapping: tp.Optional[Tensor] = None,
        embedding: tp.Optional[Tensor] = None,
        embedding_mask: tp.Optional[Tensor] = None,
        causal: tp.Optional[bool] = False
    ) -> tp.Union[tp.Tuple[Tensor, Tensor], Tensor]:
...
class UNet1d(nn.Module):
    def __init__(
        ...
        in_channels: int,
        channels: int,
        multipliers: tp.Sequence[int],
        factors: tp.Sequence[int],
        num_blocks: tp.Sequence[int],
        attentions: tp.Sequence[int],
        patch_size: int = 1,
        resnet_groups: int = 8,
        ...
        context_channels: tp.Optional[tp.Sequence[int]] = None,
        context_embedding_features: tp.Optional[int] = None,
        **kwargs,
    ):
        super().__init__()
    ...
    def get_channels(
        self, channels_list: tp.Optional[tp.Sequence[Tensor]] = None, layer: int = 0
    ) -> tp.Optional[Tensor]:
    ...
    def get_mapping(
        self, time: tp.Optional[Tensor] = None, features: tp.Optional[Tensor] = None
    ) -> tp.Optional[Tensor]:
    ...
    def forward(
        self,
        x: Tensor,
        time: tp.Optional[Tensor] = None,
        *,
        features: tp.Optional[Tensor] = None,
        channels_list: tp.Optional[tp.Sequence[Tensor]] = None,
        embedding: tp.Optional[Tensor] = None,
        embedding_mask: tp.Optional[Tensor] = None,
        causal: tp.Optional[bool] = False,
    ) -> Tensor:
...
def rand_bool(shape: tp.Any, proba: float, device: tp.Any = None) -> Tensor:
    if proba == 1:
...
class UNetCFG1d(UNet1d):
    ...
    def forward(  # type: ignore
        ...
        *,
        ...
        scale_phi: float = 0.4,
        negative_embedding: tp.Optional[Tensor] = None,
        negative_embedding_mask: tp.Optional[Tensor] = None,
        **kwargs,
    ) -> Tensor:
...
class UNetNCCA1d(UNet1d):
    ...
    def expand(self, x: tp.Any, shape: tp.Tuple[int, ...]) -> Tensor:
    ...
    def forward(  # type: ignore
        ...
        time: Tensor,
        *,
        channels_list: tp.Sequence[Tensor],
        channels_augmentation: tp.Union[
            bool, tp.Sequence[bool], tp.Sequence[tp.Sequence[bool]], Tensor
        ] = False,
        channels_scale: tp.Union[
            float, tp.Sequence[float], tp.Sequence[tp.Sequence[float]], Tensor
        ] = 0,
        **kwargs,
    ) -> Tensor:
...
class NumberEmbedder(nn.Module):
    ...
    def forward(self, x: tp.Union[tp.List[float], Tensor]) -> Tensor:
...
class STFT(nn.Module):
    ...
    def encode(self, wave: Tensor) -> tp.Tuple[Tensor, Tensor]:
        b = wave.shape[0]
    ...
    ) -> tp.Union[Tensor, tp.Tuple[Tensor, Tensor]]:
